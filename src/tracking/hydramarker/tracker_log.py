...
COLUMNS = [
    ...
    "checkerboard_lk_corner_kalman_enabled_count",
    "checkerboard_lk_corner_kalman_update_count",
    "checkerboard_lk_corner_kalman_reset_count",
    "checkerboard_lk_corner_kalman_mean_innovation_px",
    "checkerboard_lk_corner_kalman_p95_innovation_px",
    "checkerboard_lk_corner_kalman_max_innovation_px",
    "checkerboard_lk_corner_kalman_mean_correction_px",
    "checkerboard_lk_corner_kalman_p95_correction_px",
    "checkerboard_lk_corner_kalman_max_correction_px",
    "checkerboard_tracking_corner_kalman_ms",
    "checkerboard_tracking_subpix_ms",
    "checkerboard_tracking_subpix_count",
    "checkerboard_tracking_subpix_enabled_count",
    "checkerboard_tracking_subpix_mean_shift_px",
    "checkerboard_tracking_subpix_p95_shift_px",
    "checkerboard_tracking_subpix_max_shift_px",
...
def set_run_log_dir(path: Path | str) -> None:
    """Set the directory used for subsequent live tracking JSONL logs."""
    global RUN_LOG_DIR
    RUN_LOG_DIR = Path(path)


def is_active() -> bool:
    """Return whether a live tracking log is currently open."""
    return bool(_log_active)


def current_path() -> Optional[Path]:
    """Return the active log path, or None when logging is inactive."""
    return _log_path


def current_run_id() -> str:
    """Return the run identifier for the active log."""
    return str(_log_run_id)


def camera_intrinsics_info() -> dict[str, Any]:
    """Return a copy of the camera metadata written into run_start records."""
    return dict(_camera_intrinsics_info)


def set_camera_intrinsics_info(info: dict[str, Any]) -> None:
    """Replace the camera metadata written into the next run_start record."""
    global _camera_intrinsics_info
    _camera_intrinsics_info = dict(info)


def update_camera_intrinsics_info(info: dict[str, Any]) -> None:
    """Merge additional camera metadata into the next run_start record."""
    _camera_intrinsics_info.update(dict(info))


def write_record(record: dict[str, Any]) -> None:
    """Append an auxiliary JSONL record to the active run log."""
    _write_json_record(record)


def log_frame_details_enabled() -> bool:
    """Return whether detailed per-corner records should be written."""
...
def set_debug_board_transform(T_B_C: Optional[np.ndarray]) -> None:
    """Set the checkerboard-to-camera transform used for board-relative diagnostics."""
...
def has_fresh_pose(result) -> bool:
    """Return true when the current result contains a non-held pose."""
...
def log_frame(
    ...
    row = {
        ...
        "checkerboard_lk_corner_kalman_enabled_count": int(float(timings.get("checkerboard_lk_corner_kalman_enabled_count", 0.0) or 0.0)),
        "checkerboard_lk_corner_kalman_update_count": int(float(timings.get("checkerboard_lk_corner_kalman_update_count", 0.0) or 0.0)),
        "checkerboard_lk_corner_kalman_reset_count": int(float(timings.get("checkerboard_lk_corner_kalman_reset_count", 0.0) or 0.0)),
        "checkerboard_lk_corner_kalman_mean_innovation_px": _fmt_float(timings.get("checkerboard_lk_corner_kalman_mean_innovation_px")),
        "checkerboard_lk_corner_kalman_p95_innovation_px": _fmt_float(timings.get("checkerboard_lk_corner_kalman_p95_innovation_px")),
        "checkerboard_lk_corner_kalman_max_innovation_px": _fmt_float(timings.get("checkerboard_lk_corner_kalman_max_innovation_px")),
        "checkerboard_lk_corner_kalman_mean_correction_px": _fmt_float(timings.get("checkerboard_lk_corner_kalman_mean_correction_px")),
        "checkerboard_lk_corner_kalman_p95_correction_px": _fmt_float(timings.get("checkerboard_lk_corner_kalman_p95_correction_px")),
        "checkerboard_lk_corner_kalman_max_correction_px": _fmt_float(timings.get("checkerboard_lk_corner_kalman_max_correction_px")),
        "checkerboard_tracking_corner_kalman_ms": _fmt_float(timings.get("checkerboard_tracking_corner_kalman_ms")),
        ...
        "checkerboard_tracking_subpix_enabled_count": int(float(timings.get("checkerboard_tracking_subpix_enabled_count", 0.0) or 0.0)),
        "checkerboard_tracking_subpix_mean_shift_px": _fmt_float(timings.get("checkerboard_tracking_subpix_mean_shift_px")),
        "checkerboard_tracking_subpix_p95_shift_px": _fmt_float(timings.get("checkerboard_tracking_subpix_p95_shift_px")),
        "checkerboard_tracking_subpix_max_shift_px": _fmt_float(timings.get("checkerboard_tracking_subpix_max_shift_px")),
