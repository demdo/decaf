...
Mat1b generator_HydraMarker::get_field() const
{
	return this->field.clone();
}

Mat1b generator_HydraMarker::generate_planar_field(
	int rows,
	int cols,
	int patch_size,
	double max_ms,
	...
	if (rows <= 0 || cols <= 0)
	{
		CV_Error(Error::StsBadArg, "rows and cols must be positive");
	}

	if (patch_size <= 0)
	{
		CV_Error(Error::StsBadArg, "patch_size must be positive");
	}

	if (patch_size > rows || patch_size > cols)
	{
		CV_Error(Error::StsBadArg, "patch_size must be <= rows and <= cols");
	}

	generator_HydraMarker generator;

	Mat1b state = 2 * Mat1b::ones(rows, cols);
	Mat1b shape = Mat1b::ones(patch_size, patch_size);

	generator.set_field(state);
	generator.set_tagShape(shape);
	...
	Mat1b result = generator.get_field();

	if (result.empty())
	{
		CV_Error(Error::StsError, "generated marker field is empty");
	}
	...
	for (int r = 0; r < result.rows; ++r)
	{
		...
		{
			uchar value = result(r, c);
			if (value != 0 && value != 1)
			{
				CV_Error(Error::StsError, "generated marker still contains unresolved values");
			}
		}
	}
