...
public:
    ...
    void generate(
        const METHOD method = METHOD::FBWFC,
        const double max_ms = 3600000000,
        const int max_trial = INT_MAX,
        const bool show_process = true,
        const string path = "process.log"
    );
    ...
    // return current marker field as binary/generated state matrix
    Mat1b get_field() const;

    // Convenience function for planar binary HydraMarker generation.
    // Creates rows x cols unknown field, uses a patch_size x patch_size solid tag shape,
    // runs FBWFC, and returns the generated 0/1 field.
    static Mat1b generate_planar_field(
        int rows,
        int cols,
        int patch_size,
