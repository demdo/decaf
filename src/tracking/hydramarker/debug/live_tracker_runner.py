...
# start, while still returning to a cheap video-only idle state.
...
def draw_status(vis: np.ndarray, result, frame_idx: int, tracker: HydraTracker) -> None:
    ...
    put_text(vis,
             "blue=detector | green=global corr | magenta=reprojection | s=start/stop | r=reset | q=quit",
