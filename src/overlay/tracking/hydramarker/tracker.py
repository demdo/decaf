...
class TrackerConfig:
    ...
    min_fallback_identities: int = 15
    fallback_requires_tracking: bool = True
    fallback_requires_stable: bool = True
...
class HydraTracker:
    ...
    def __init__(
        ...
        self.correspondence_builder = (
            self._create_correspondence_builder()
        )
    ...
    def _create_patch_decoder(self):
        ...
        cfg.require_geometry_valid = (
            self.config.decoder_require_geometry_valid
        )

        cfg.accept_ambiguous = (
            self.config.decoder_accept_ambiguous
        )
    ...
    def _create_correspondence_builder(self):
        ...
        cfg.discard_conflicts = (
            self.config.corr_discard_conflicts
        )

        cfg.require_detection_stable = (
            self.config.corr_require_detection_stable
        )
    ...
    def process_frame(self, frame: np.ndarray) -> TrackerResult:
        ...
        if self.config.enable_fallback and self.identities:
            fallback = self._fallback_pose_from_known_identities(
                detection
            )
        ...
        if self.lost_frames > self.config.max_lost_frames:
            ...
            self.mode = (
                TrackerMode.RECOVERING
                if self.identities
                else TrackerMode.DETECTING
            )
    ...
    def _lost_result(
        ...
        return TrackerResult(
            ...
            else bool(detection.tracking),
            detection_stable=False
            if detection is None
            else bool(detection.stable),
        )
    ...
    def _full_decode_pose(
        self,
        frame: np.ndarray,
        detection,
    ) -> TrackerResult:

        dots = self.dot_detector.detect(
            frame,
            detection,
        )
        ...
        pose.message = (
            "Full decode + pose solver successful."
        )

        return pose

    def _fallback_pose_from_known_identities(
        self,
        detection,
    ) -> TrackerResult:
    ...
    def _estimate_pose_from_identities(
        ...
        raw = solve_pose(
            ...
            ransac_reprojection_error_px=(
                self.config.pnp_ransac_reprojection_px
            ),
            ransac_confidence=(
                self.config.pnp_ransac_confidence
            ),
            ransac_iterations_count=(
                self.config.pnp_ransac_iterations
            ),
        )
        ...
        return TrackerResult(
            ...
            message="Pose solver successful.",
            corners=identities,
            rvec=rvec,
            tvec=tvec,
            T_marker_camera=T,
            ...
            num_inliers=len(raw.inlier_idx),
        )
